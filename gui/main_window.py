...
class CyberSecurityTool:
    ...
    def _update_stats_loop(self):
        if self.monitor.monitoring:
            ...
            stats_display = (f"Uptime: {stats['uptime']}\n"
                             ...
                             f"  - ML Anomalies: {stats['ml_anomalies']}\n\n"
    ...
    def _update_charts(self, stats):
        ...
        if stats['threats_detected'] > 0:
            ...
            sizes = [stats['dos_count'], stats['port_scan_count'], stats['ml_anomalies']]
    ...
    def _show_port_scanner(self):
        ...
        start_port = ttk.Entry(top_frame, width=6)
        start_port.pack(side=tk.LEFT, padx=2)
        start_port.insert(0, "1")
        end_port = ttk.Entry(top_frame, width=6)
        end_port.pack(side=tk.LEFT, padx=2)
        end_port.insert(0, "1024")
    ...
    def _run_port_scan(self, ip, start, end, results_widget):
        try:
            ...
            results_widget.insert(tk.END, f"Scanning {ip} ports {start}-{end}...\n")
    ...
    def _show_bandwidth_monitor(self):
        """Show bandwidth monitor window"""
        try:
            from gui.bandwidth_window import BandwidthWindow
            BandwidthWindow(self.root, self.monitor)
        except ImportError as e:
            messagebox.showerror("Error", f"Bandwidth Monitor not available: {e}")

    def _show_threat_intel(self):
        """Show threat intelligence window"""
        try:
            from gui.threat_intel_window import ThreatIntelWindow
            ThreatIntelWindow(self.root, self.monitor)
        except ImportError as e:
            messagebox.showerror("Error", f"Threat Intelligence not available: {e}")

    def _show_firewall(self):
        """Show firewall manager window"""
        try:
            from gui.firewall_window import FirewallWindow
            FirewallWindow(self.root, self.monitor)
        except ImportError as e:
            messagebox.showerror("Error", f"Firewall Manager not available: {e}")

    def _show_ml_dashboard(self):
        """Show ML dashboard window"""
        try:
            from gui.ml_dashboard import MLDashboard
            MLDashboard(self.root, self.monitor)
        except ImportError as e:
            messagebox.showerror("Error", f"ML Dashboard not available: {e}")

    def _show_about(self):
        about_text = f"""Realtime Network Threat Detection v{VERSION}

An educational tool for demonstrating network monitoring 
...
This tool is designed for learning purposes and 
authorized network security testing only.

© 2024 Network Security Education Project"""
...
1. Enter Target IP
   - Enter the IP address you want to monitor
   - Can be local (192.168.x.x) or remote
...
4. Advanced Tools
   - Tools → Port Scanner: Scan for open ports
   - Tools → Packet Analyzer: Deep packet inspection
   - Advanced → Threat Intelligence: View threat reports
   - Advanced → Firewall Manager: Manage blocking rules
   - Advanced → ML Dashboard: View ML predictions
